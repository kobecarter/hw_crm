...
class item_devis
{
    static $table =  __prefixe_db__ . "item_devis";
    ...
    public function add()
    {
        global $db;
        $SQLinsert = sprintf(
            "INSERT INTO " . static::$table . " (id_devis, id_service, qte, discount, prix, total, unite, titre, description, ordre) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
    ...
    public function edit()
    {
        global $db;
        $SQLupdate = sprintf(
            "UPDATE " . static::$table . " SET  id_devis = %s, id_service = %s, qte = %s, discount = %s, prix = %s, total = %s, unite = %s, titre = %s, description = %s, ordre = %s  WHERE id = %s",
            GetSQLValueString($this->devis->getId(), "int"),
            GetSQLValueString($this->service->getId(), "int"),
            GetSQLValueString($this->qte, "int"),
            GetSQLValueString($this->discount, "double"),
    ...
    public static function findAllByDevis($id_devis)
    {
        ...
        $items = array();
        $SQLselect = sprintf(
            "SELECT * FROM " . static::$table . " WHERE id_devis = %s ORDER BY date_add DESC, id DESC",
    ...
    {
        $item_devis = new item_devis();
        $item_devis->setId($data['id']);
        $item_devis->setDevis(isset($data['id_devis']) ? devis::find($data['id_devis'], $_SESSION['agence']) : null);
        $item_devis->setService(isset($data['id_service']) ? service::find($data['id_service']) : null);
        $item_devis->setQte($data['qte']);
        $item_devis->setDiscount($data['discount']);
        $item_devis->setPrix($data['prix']);
        $item_devis->setTotal($data['total']);
        $item_devis->setUnite($data['unite']);
        $item_devis->setTitre($data['titre']);
        $item_devis->setDescription($data['description']);
        $item_devis->setOrdre($data['ordre']);
        return $item_devis;
    }
    ...
    public static function findByIdApi($id)
    {
        ...
        if ($db->num_rows($result) == 1) {
            $data = $db->fetch_assoc($result);
            $item_devis = static::buildApi($data);
        }
        return $item_devis;
    }

    public static function findAllByDevisApi($id_devis)
    {
        global $db;
        $items = array();
        $SQLselect = sprintf(
