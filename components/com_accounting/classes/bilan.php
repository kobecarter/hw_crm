...
class bilan
{
    static $table =  __prefixe_db__ . "bilan";
    ...
    public static function findAll($agence = 1)
    {
        ...
        foreach ($result as $data) {
            $bilan = static::build($data);
            array_push($items, $bilan);
        }
        return $items;
    }

    // Utilisé par la vérification anti-doublon de la dropzone Charges : existe-t-il déjà un
    // bilan enregistré pour cette année et cette agence ?
    ...
    public static function build($data){
        ...
        $bilan->setDoc($data['doc']);
        $bilan->setDateAdd($data['date_add']);
        $bilan->setLastEdit($data['last_edit']);
        return $bilan;
    }
