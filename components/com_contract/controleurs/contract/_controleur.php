...
function pdfContract($data){
    ...
    if (isset($data["id"]) && !empty($data["id"])) {
        ...
        if($devis->getLangue() == 'fr'){
            ...
            <p style="margin-top:30px"><b>'.$client->getRaisonSocial().'</b>, immatriculée au Registre du commerce de '.$agence->getVille().' sous le numéro '.$client->getRc().', dont le siège social est <b>'.$client->getAdresse().'</b> et représentée par <b>'.$client->getTitre().'. '.$client->getNom() .' '.$client->getPrenom() .'</b> agissant en sa qualité de '.($client->getFonction() ? $client->getFonction() : '......') .' dûment habilité à signer les présentes, </p>
            ...
            <p style="margin-top:20px">Le délai de livraison site web interviendra deux mois après la signature du <b>CONTRAT</b>.</p>
            ';
            ...
            <p style="margin-top:20px">La société <b>'.$agence->getRaisonSocial().'</b> mettra à disposition de la société <b>'.$client->getRaisonSocial().'</b> l’ensemble de ses équipes <b>(technique, artistique, marketing…etc.)</b> ainsi que les moyens nécessaires pour le bon déroulement du partenariat à partir du <u><b>'.normalDate(date('Y-m-d',strtotime($contract->getDate().' +7 days'))) .'</b></u>. (Date prévisionnelle de début de contrat).</p>
            ...
            <htmlpageheader name="myheader">
                ...
                if($contract->getGarantie()){
                    $htmlInvoices .= '<h2 style="margin-top:25px"><u>10 - GUARANTEE </u></h2>
                        <ul style="margin-top:20px">
                    ...
                    $htmlInvoices .= '<h2 style="margin-top:25px"><u>10 - RESPONSABILITY </u></h2>
                ...
                $htmlInvoices .= '<h2 style="margin-top:25px"><u>11 - PRIVACY </u></h2>
